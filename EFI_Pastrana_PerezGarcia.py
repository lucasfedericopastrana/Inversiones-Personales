...
class MyApp(App):
    def OnInit(self):
        ...
        self.image = image = StaticBitmap(p, -1, Bitmap("C:\EFI Pastrana_PerezGarcia\imagen.png", BITMAP_TYPE_ANY))
    ...
    def instrucciones(self,event):
        ...
        p = Panel(f)
        s = BoxSizer(VERTICAL)
        image = StaticBitmap(p, -1, Bitmap("C:\EFI Pastrana_PerezGarcia\instrucciones.jpg", BITMAP_TYPE_ANY))
        s.Add(image, 0, ALL|EXPAND, 20)
        p.SetSizer(s)
        f.Show()
        return True
